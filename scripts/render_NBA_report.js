...
const nbaColorData = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "data", "NBA_team_colors.json"), "utf8"));

async function renderNbaReport(gameId, roundText = "") {
    const dataPath = path.join(__dirname, "..", "data", "reports", `report_nba_${gameId}.json`);
    ...
    let html = fs.readFileSync(path.join(__dirname, "..", "template", "NBA_report.html"), "utf8");

    const pct = (m, a) => (a > 0 ? ((m / a) * 100).toFixed(1) : "0.0");
    const ratio = (val, total) => {
    const res = (total > 0 ? Math.round((val / total) * 100) : 0);
    return isNaN(res) ? 0 : res; // 安全策
};
    ...
    // キーワード置換マップ
    const reps = {
        // 色の指定を厳密化（__COLOR2__ が __COLOR__ を含むため、置換順序が重要）
...
sortedKeys.forEach(k => {
    ...
    await page.evaluate(({ show }) => {
        ...
        if (badge && !show) {
            badge.style.display = 'none'; // テキストが空なら要素ごと消す
        }
    }, { show: hasRoundText });
    ...
    const outPath = path.join(__dirname, "..", "output", "reports");
