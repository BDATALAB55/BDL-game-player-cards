        ...
        const stats = await page.evaluate(() => {
            ...
            const getSideData = (table) => {
                ...
                if (teamRow) {
                    ...
                    if (c.length >= 30) {
                        // TEAM行専用のインデックスで取得
                        // 18:OR, 19:DR, 20:TR, 23:TOV が一般的なTEAM行の並びです
                        const teamOreb = parseInt(c[18]) || 0;
                        const teamDreb = parseInt(c[19]) || 0;
                        const teamReb  = parseInt(c[20]) || 0;
                        ...
                        total.oreb += teamOreb;
                        total.dreb += teamDreb;
                        total.reb  = total.oreb + total.dreb; // 合計を再計算して整合性を保つ
                        total.tov  += teamTov;
                        total.pf   += teamPf;
                    }
                }

                return { total, players: rows.filter(p => !p.nameJp.includes("TEAM")) }; // 選手リストからはTEAMを除外
            };
            return {
                    ...
                    if (p.detailUrl) {
                        ...
                        try {
                            ...
                            let rawName = await pPage.evaluate(() => {
                                ...
                                if (!m) return null;
                                // PPGなどの余計な文字を削除してクリーンにする
                                return m[1].split(/\s(?:PPG|APG|RPG|BPG|SPG)/)[0].replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
                            });

                            if (rawName) {
                                // 【重要】ここで全大文字を Title Case に強制変換
                                // HIROMU NAKAMURA -> Hiromu Nakamura
                                p.name = rawName.toLowerCase().split(' ').map(word => {
                                    // 1文字目を大文字に、残りをそのまま（小文字化したもの）
                                    return word.charAt(0).toUpperCase() + word.slice(1);
                                }).join(' ');
                                
                                // 特殊ケース微調整: "D.j." -> "D.J." のようにドット直後を修正したい場合はここ
