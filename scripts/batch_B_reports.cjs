...
if (gameIds.length === 0) {
    ...
    console.log("例: node scripts/batch_B_reports.cjs 505698 505694 505690");
    process.exit(1);
}
...
async function runBatch() {
    ...
    for (let i = 0; i < gameIds.length; i++) {
        ...
        await new Promise((resolve) => {
            ...
            // 【修正ポイント】実行するファイル名を .cjs に変更
            const child = spawn('node', ['scripts/run_B_report.cjs', id], { stdio: 'inherit' });
